...
const handleSubmit = async () => {
    ...
    try {
        ...
        toast.add({
            ...
            detail:   `${form.value.name} created successfully. Password setup email sent.`,
            life:     4000
        })
        ...
        if (data?.errors) {
            ...
            // ✅ Also show toast for any errors not tied to form fields
            const knownFields = [
                'name', 'email', 'phone', 'type',
                'designation', 'meta'
            ]
            const unknownErrors = Object.keys(data.errors)
...
<template>
    <div>
        ...
        <div class="flex flex-col gap-4">
            ...
            <!-- ─── Section 2: Agency/Additional Info ── -->
            <div class="card">
                <div v-if="isAgentType" class="font-semibold text-xl mb-6">Agency Information</div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">

                    <div class="flex flex-col gap-2">
                        <label class="font-medium text-sm">
                            {{ isAgentType ? 'Agency Name' : 'Company Name' }}
                        </label>
                        <InputText
                            v-model="form.meta.company_name"
                            :placeholder="isAgentType ? 'Enter agency name' : 'Enter company name'"
                        />
                    </div>
